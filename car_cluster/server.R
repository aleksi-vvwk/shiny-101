...
shinyServer(function(input, output) {
  ...
  toinclude <-
    c("mpg", "disp", "hp", "wt", "drat", "qsec") # selected variables
  standcars <-
    sapply(subset(mtcars, select = toinclude), "stand")       # apply fun
  rownames(standcars) <-
    rownames(mtcars)                              # car names
  ...
  output$distPlot <- renderPlot({
    ...
    if (sum(tocluster) == 0) {
      ...
      dmat <-
        dist(standcars[, tocluster], method = "euclidean")                   # distances
      fit <-
        hclust(dmat, method = "ward.D")                           # Hierarchical clustering
      plot(
        fit,
        main = paste(
          "Cluster Dendrogram. Mean height (i.e. distance):",
          round(mean(dmat), 1)
        ),
        xlab = "You can choose how many clusters and which variables"
      )    # Display dendogram
